import { doctors } from './profiles/index.js';
...
const modalCopy={
  ...
  en:{close:'Close',verified:'Verified information',public:'Public information',sample:'Sample profile',hoursNote:'Opening hours have not yet been confirmed directly with the clinic',contact:'Contact',location:'Clinic location',missingTitle:'Demonstration content only',missingBody:'The overview, education, services, languages, insurance, and accessibility were invented for design purposes and are not verified information about Dr. Karam Samara.',sampleTitle:'Demonstration content only',sampleBody:'The overview, education, services, languages, insurance, and accessibility were invented for design purposes and are not verified information about Dr. Karam Samara.',noWebsite:'No official website published'}
};
modalCopy.he.clinicVerified='פרטים שאומתו מול המרפאה';
modalCopy.ar.clinicVerified='معلومات تم التحقق منها مع العيادة';
modalCopy.en.clinicVerified='Details confirmed by the clinic';
...
function currentAddress(doctor){ return label(doctor.address?.[state.city] || doctor.address); }
function currentMap(doctor){ return typeof doctor.maps==='string'?doctor.maps:doctor.maps?.[state.city] || doctor.maps?.default || ''; }
...
function renderClinics(){
  ...
  const matches=doctors.filter(doctor=>{
    const doctorCities=Array.isArray(doctor.city)?doctor.city:[doctor.city];
    const doctorBranches=Array.isArray(doctor.branch)?doctor.branch:[doctor.branch];
    return doctorCities.includes(state.city)&&doctor.field===state.field&&doctorBranches.includes(state.branch)&&(!requiresSubcategory||doctor.subcategories?.includes(state.subcategory));
  });
  ...
  matches.forEach(doctor=>{
    ...
    const badge=doctor.clinicVerified?copy.clinicVerified:doctor.sampleProfile?copy.demo:doctor.real?copy.public:copy.demo;
    const specialties=doctor.cardSpecialties?.map(item=>label(item)).join(' · ');
    const avatar=doctor.photo?`<img src="${doctor.photo}" alt="">`:doctor.initials;
    entry.innerHTML=`<button class="clinic-row"><span class="clinic-row-avatar">${avatar}</span><span class="clinic-row-main"><small>${badge}</small><strong>${label(doctor.name)}</strong><span class="clinic-row-field">${label(doctor.detail)}</span>${specialties?`<span class="clinic-row-focus"><b>${copy.focusLabel}:</b> ${specialties}</span>`:''}</span><span class="row-arrow">←</span></button>`;
...
function openProfile(doctor){
  ...
  const address=currentAddress(doctor);
  const maps=currentMap(doctor);
  const phones=(Array.isArray(doctor.phone)?doctor.phone:[doctor.phone]).filter(Boolean);
  const primaryPhone=phones[0];
  const phoneContacts=phones.map(phone=>`<a class="contact-line" href="tel:${phone.replaceAll('-','').replaceAll(' ','')}"><span>☎</span><div><small>${copy.call}</small><strong dir="ltr">${phone}</strong></div></a>`).join('');
  ...
  profileModalContent.innerHTML=`<header class="modal-hero">${modalAvatar}<div class="modal-identity"><span>${doctor.clinicVerified?modal.clinicVerified:doctor.real?modal.public:modal.verified}</span><h2 id="profile-modal-title">${label(doctor.name)}</h2><p>${label(doctor.detail)}</p><small>⌖ ${address}</small></div><div class="modal-actions">${primaryPhone?`<a class="action-primary" href="tel:${primaryPhone.replaceAll('-','').replaceAll(' ','')}">☎ ${copy.call}</a>`:''}${maps?`<a href="${maps}" target="_blank" rel="noopener">⌖ ${copy.navigate}</a>`:''}${doctor.website?`<a href="${doctor.website}" target="_blank" rel="noopener">↗ ${profile.website}</a>`:''}</div></header><div class="modal-body"><main class="profile-main-column"><section class="profile-details-card"><div class="profile-detail-row"><div class="section-icon">◌</div><div><h3>${profile.overview}</h3>${doctor.overview?`<p>${label(doctor.overview)}</p>`:`<p class="section-empty">${profile.missing}</p>`}</div></div><div class="profile-detail-row"><div class="section-icon">◇</div><div><h3>${profile.education}</h3>${chips(doctor.education)}</div></div><div class="profile-detail-row"><div class="section-icon">✓</div><div><h3>${profile.credentials}</h3>${chips(doctor.credentials)}</div></div></section><section class="modern-section service-section"><div class="section-icon">+</div><div><h3>${profile.services}</h3>${chips(doctor.services)}</div></section></main><aside class="profile-side-column"><section class="side-card hours-card"><div class="side-card-title"><div><span>◷</span><h3>${profile.hours}</h3></div>${doctor.hoursVerified?'':`<small>${modal.public}</small>`}</div><div class="weekly-hours">${schedule}</div>${doctor.hoursVerified?'':`<p class="hours-note">${modal.hoursNote}</p>`}</section><section class="side-card"><h3>${modal.contact}</h3>${phoneContacts}${doctor.email?`<a class="contact-line" href="mailto:${doctor.email}"><span>@</span><div><small>${profile.email}</small><strong dir="ltr">${doctor.email}</strong></div></a>`:''}${maps?`<a class="contact-line" href="${maps}" target="_blank" rel="noopener"><span>⌖</span><div><small>${modal.location}</small><strong>${address}</strong></div></a>`:''}${doctor.website?`<a class="contact-line" href="${doctor.website}" target="_blank" rel="noopener"><span>↗</span><div><small>${profile.website}</small><strong>${doctor.websiteName?label(doctor.websiteName):profile.website}</strong></div></a>`:`<div class="website-empty">${modal.noWebsite}</div>`}</section></aside></div>`;
